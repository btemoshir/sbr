"""
Collection of functions which implement different levels of approximation for the diagrammatic approach to the coagulation reaction A+A -> A with A <-> \nullset.
...
def integrate_alpha2_alpha_2(k1,k2,k3,alpha=1.,init_time=0.,final_time=1.,dt=0.001,initial_values=1.):
    ...
    for i in range(len(time_grid)-1):
        ...
        dydt       = k1[0] - k2[0]*y[i] -alpha*k3[0]*y[i]**2 + 2*(alpha*k3[0])**2*dt*np.sum(resp[i,1:i+1]**2*y[0:i]**2)
        y[i+1]     = y[i] + dydt*dt
...
def integrate_all_alpha_0(k1,k2,k3,alpha=1.,init_time=0.,final_time=1.,dt=0.001,initial_values=1.):
    ...
    for i in range(len(time_grid)-1):
        ...
        dydt       = k1[0] - k2[0]*y[i] -alpha*k3[0]*y[i]**2 + 2*(alpha*k3[0])**2*dt*np.sum(temp_mat[i,1:i+1]*y[0:i]**2)
...
def integrate_alpha2_alpha_all(k1,k2,k3,alpha=1.,init_time=0.,final_time=1.,dt=0.001,initial_values=1.):
    ...
    for i in range(len(time_grid)-1):
        ...
        for j in range(i+1):
            dRdt[j] = -k2*resp[i,j] -2*alpha*k3[0]*y[i]*resp[i,j]
            if j < i:
                dRdt[j] += 4*(alpha*k3[0])**2*dt*np.sum(temp_mat[i,j+1:i+1]*y[j:i]*resp[j:i,j])
...
def integrate_mak(k1,k2,k3,alpha=1.,init_time=0.,final_time=1.,dt=0.001,initial_values=1.):
    ...
    for i in range(len(time_grid)-1):
        ...
        dydt       = k1[0] - k2[0]*y[i] -alpha*k3[0]*y[i]**2
        y[i+1]     = y[i] + dydt*dt
...
def emre(k1,k2,k3,alpha=1.,init_time=0.,final_time=1.,dt=0.001,initial_values=1.):
    ...
    #var[0]    = initial_values**2 + initial_values
    #var[0]    = 0
    
    #resp      = np.zeros([len(time_grid),len(time_grid)])
    #resp[0,0] = 1.
    #corr      = np.zeros([len(time_grid),len(time_grid)])
    
    for i in range(len(time_grid)-1):
        
        #dydt       = k1[0] - k2[0]*y[i] -alpha*k3[0]*y[i]**2
        #depsdt     = -k2[0]*eps[i] -2*alpha*k3[0]*y[i]*eps[i] -alpha*k3[0]*(var[i] - y[i])
        #dvardt     = -2*k2[0]*var[i] -4*alpha*k3[0]*y[i]*var[i] + k1 + k2*y[i] +alpha*k3[0]*y[i]**2
